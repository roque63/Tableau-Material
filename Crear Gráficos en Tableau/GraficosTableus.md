

# Crear un gráfico de dispersión

- https://help.tableau.com/current/pro/desktop/en-us/buildexamples_scatter.htm

![Texto](https://help.tableau.com/current/pro/desktop/en-us/Img/build_scatter2.gif)

![texto del vínculo](build_scatter2.gif)



# Crear un gráfico de barras
- https://help.tableau.com/current/prodesktop/en-us/buildexamples_bar.htm


![texto del vínculo](https://help.tableau.com/current/pro/desktop/en-us/Img/barchart1.gif)


# Crear un diagrama de caja 
- https://help.tableau.com/current/pro/desktop/en-us/buildexamples_boxplot.htm


![texto del vínculo](https://help.tableau.com/current/pro/desktop/en-us/Img/boxchart1.gif)


# Construir con marcas de densidad 
- https://help.tableau.com/current/pro/desktop/en-us/buildexamples_density.htm


# Crear un diagrama de Gantt

- https://help.tableau.com/current/pro/desktop/en-us/buildexamples_gantt.htm

![texto del vínculo](https://help.tableau.com/current/pro/desktop/en-us/Img/ganttchart1.gif)

# Crear una tabla de puntos destacados o un mapa de calor
- https://help.tableau.com/current/pro/desktop/en-us/buildexamples_highlight.htm

![texto del vínculo](https://help.tableau.com/current/pro/desktop/en-us/Img/highlightchart1.gif)

# Construir un histograma 
- https://help.tableau.com/current/pro/desktop/en-us/buildexamples_histogram.htm

![texto del vínculo](https://help.tableau.com/current/pro/desktop/en-us/Img/histogramchart1.gif)

# Creación de gráficos de líneas
- https://help.tableau.com/current/pro/desktop/en-us/buildexamples_line.htm

![texto del vínculo](https://help.tableau.com/current/pro/desktop/en-us/Img/linechart1.gif)


# Creación de gráficos de burbujas
- https://help.tableau.com/current/pro/desktop/en-us/buildexamples_bubbles.htm

![texto del vínculo](https://help.tableau.com/current/pro/desktop/en-us/Img/bubblechart1.gif)

# Creación de un gráfico circular
- https://help.tableau.com/current/pro/desktop/en-us/buildexamples_pie.htm

![texto del vínculo](piechart-checkwork.gif)


# Construir un mapa de árbol

https://help.tableau.com/current/pro/desktop/en-us/buildexamples_treemap.htm

![texto del vínculo](treemapchart1.gif)

# Construir un gráfico de pareto

https://help.tableau.com/current/pro/desktop/en-us/pareto.htm

